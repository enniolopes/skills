...
**The error it prevents.** A manuscript reviewed only by the person who wrote it; a
checklist item skipped silently; a fix that changed a result nobody re-rendered.

**Exit gate.** Reviewer verdict `PASS`, or every `FAIL` has a logged response and the
affected checks have been re-run.
