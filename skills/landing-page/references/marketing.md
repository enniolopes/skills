...
- specific enough to exclude unrelated products;
- connected to a real capability or offer;
- intelligible without internal company language;
- stronger because of evidence, not because of adjectives.

Prefer concrete consequence over generic aspiration.

Weak: `Transform your workflow with the future of AI.`

Stronger pattern: `Turn [painful current process] into [observable better state] using [credible mechanism].`

Do not force this verbal formula when a different expression is better. The principle is causal specificity.
...
- How does it work?
- Why is this different from what I already have?
- Can I believe this?
...
Write for comprehension and conviction, not copywriter performance.
...
- empty adjectives (`powerful`, `seamless`, `innovative`) without substance;
- overly symmetrical AI copy (`faster. smarter. better.`) unless genuinely suited;
- clever labels that obscure action;
- duplicated explanations across sections;
- paragraph volume compensating for weak hierarchy;
...
Do not require conventional blocks. Structure is a projection of the visitor states that remain unresolved for this page intent.

Examples:

- A known product with high purchase intent may need `offer → proof → terms → action`.
- A novel category may need `problem → mechanism → product → proof → action`.
...
These are points generated by the model, not templates. The structure is successful when each transition is necessary and the full page accumulates conviction without repetition.
...
When the page-intent basis is incomplete:
...
Do not ask the user questions such as `Which sections do you want?`, `What persuasion framework should we use?`, or `Should the hero be product-led or benefit-led?` unless they explicitly want to direct those decisions.

When multiple truthful messages are possible, prefer the one with the strongest combination of:

`audience relevance × product specificity × evidence strength × decision impact`

Do not overfit the page to speculative personas or invented customer psychology. A compact, evidence-backed decision model is better than a detailed fictional persona.
